...
def HiCAT_inputs_gen(input_files_dict):
	""" Generate HiCAT aperture and Lyot stop(s) for the APLC coronagraph.

	    Parameters
	    ----------
	    input_files_dict: dict
	        A dictionary of input parameters provided in `do_hicat` launcher script.

	    Returns
	    -------
	    pup_filename: list
	        name of the generated aperture file.
	    ls_filename: list
	        name of the generated Lyot stop file.
	    """
	...
	pup_filename = filepath+'ApodMask_HiCAT_ovsamp{0:02d}_N{1:04d}_Spider{2:}_Gap{3:}.fits'.format(oversamp, N,ap_spid,ap_gap)																								  N)
	...
	if config.is_file():
		...
		HiCAT_ap, header           = make_hicat_aperture(with_spiders=ap_spid, with_segment_gaps=ap_gap, return_header=True)
