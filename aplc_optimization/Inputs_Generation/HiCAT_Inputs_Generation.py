...
def HiCAT_inputs_gen(input_files_dict):
    ...
    pup_filename = filepath + 'ApodMask_HiCAT_ovsamp{0:02d}_N{1:04d}_Spider{2:}_Gap{3:}.fits'.format(oversamp, N,
                                                                                                     ap_spid, ap_gap)
    ...
    for ls_od in LS_OD:
        for ls_id in LS_ID:
            ...
            ls_filename = filepath + 'LS_HiCAT_ID{0:04d}_OD{1:04d}_Spider{2:}_ovsamp{3:02d}_N{4:04d}.fits'.format(
                int(ls_id * 1000),
                int(ls_od * 1000),
                ls_spid, oversamp, N)
            ...
            if config.is_file():
                ...
                HICAT_ls, ls_header = make_a_hicat_lyot_stop(lyot_inner=ls_id, lyot_outer=ls_od, with_spiders=ls_spid,
                                                             return_header=True)

                magnification_tolerance = 0.000  # not investigated yet, in case we want to add robustness to
                # magnification later
