...
def make_SCDA_hex_aperture(normalized=True, with_spiders=False, with_obstruction=True, clipped=False,
    ...
    num_rings : int
        ...
        Default is 6.
    clipped : bool
        Maximize the telescope geometry to render it as circular a possible by removing the corner segments and
    ...
    if clipped:
        ...
        # re-arranged and solving quadratic equation for r_n (center to corner)
    ...
    if clipped:
        if num_rings > 5:
            segment_positions = segment_positions.subset(circular_aperture(pupil_diameter * 0.93))
        else:
            segment_positions = segment_positions.subset(circular_aperture(pupil_diameter * 0.92))
